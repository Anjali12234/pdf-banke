...
@section('main-container')
    <section class="container-fluid sections">
        ...
        <div class="bar mt-3">
            ...
            <strong> {{ $documentCategory->title }} </strong>
        ...
        <div class="row ">
            <div class="col-md-12">
                <table class="table bg-white mb-4" id="karyavidhi">
                    ...
                    <tbody>
                        @foreach ($documentCategory->documentLists as $key => $documentList)
                            <tr>
                                <th scope="row">{{ $loop->iteration }}</th>
                                <td>{{ $documentList->title??'' }}</td>
                                <td> {{ $documentList->documentCategory->title??'' }}</td>
                                <td> {{ $documentList->publish_date??'' }}</td>
                                <td> PDF</td>
                                <td><a href="{{ route('documentDetail', $documentList) }}"><i class="fa fa-eye btn btn-primary btn-xs">
