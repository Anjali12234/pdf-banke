    ...
    <section class="container-fluid sections">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ $downloadCategory->title }}</li>
            </ol>
        </nav>
        <div class="bar mt-3">
            <i class="fa fa-clipboard mt-3 " aria-hidden="true"></i>
            <strong> {{ $downloadCategory->title }} </strong>
                    ...
                    <tbody>
                        @foreach ($downloadCategory->downloadLists as $key => $downloadList)
                            <tr>
                                ...
                                <td> {{ $downloadList->publish_date ?? '' }}</td>
                                <td> PDF</td>
                                <td><a href="{{ route('downloadDetail', $downloadList) }}"><i
